...
void FanoronaWidget::onEndTurnClicked()
{
    ...
    // دریافت وضعیت بورد به جای فراخوانی متد مستقیم
    ...
    QVariantMap move;
    move["endTurn"] = true;
    m_game->makeMove(m_game->currentPlayer(), move);
}
...
void FanoronaWidget::drawPieces(QPainter& p) const
{
    ...
    for (int i = 0; i < ROWS * COLS; ++i) {
        ...
        // اصلاح رنگ‌ها بر اساس قوانین رسمی (۱ = خاکستری، ۲ = نارنجی)
        p.setBrush(val == 1 ? QColor(128, 128, 128) : QColor(255, 140, 0));
